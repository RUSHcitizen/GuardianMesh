...
class TemporalFeatures:
    ...
    posture_change: float = 0.0  # recent upright-to-horizontal change in degrees
...
class TemporalAnalyzer:
    ...
    def extract_features(
        ...
        posture_change = self._compute_posture_change(poses)
    ...
    def _compute_vertical_drop(self, poses: List[Pose], h: int, w: int) -> float:
        ...
        # Use the recent window so a fall is not diluted by the full history.
        recent_positions = head_positions[-(self.velocity_window + 1):]
        y_trend = np.polyfit(range(len(recent_positions)), recent_positions, 1)[0]
    ...
    def _compute_posture_change(self, poses: List[Pose]) -> float:
        """Measure a recent upright-to-horizontal posture transition."""
        angles = [self._compute_body_angle(pose) for pose in poses]
        if len(angles) < 2:
            return 0.0

        recent_start = max(0, len(angles) - self.velocity_window - 1)
        return float(np.clip(angles[-1] - angles[recent_start], 0.0, 90.0))
...
class FallDetector:
    ...
    def compute_fall_score(
        ...
        signals['posture_change'] = min(1.0, features.posture_change / 45.0)
        ...
        fall_score = (
            signals['body_angle'] * 0.20 +
            signals['posture_change'] * 0.25 +
            signals['ground_contact'] * 0.20 +
            signals['vertical_drop'] * 0.20 +
            signals['impact_pattern'] * 0.10 +
            signals['immobility'] * 0.05
        )

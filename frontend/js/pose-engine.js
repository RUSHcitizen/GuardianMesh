...
export function createPoseEngine() {
  ...
  /** Derive temporal features from consecutive keypoint samples. */
  function deriveFeatures(track, points, dtMs) {
    const byName = Object.fromEntries(points.map((p) => [p.name, p]));
    const hipY = (byName.left_hip.y + byName.right_hip.y) / 2;
    const hipX = (byName.left_hip.x + byName.right_hip.x) / 2;
    const shoulderY = (byName.left_shoulder.y + byName.right_shoulder.y) / 2;
    const shoulderX = (byName.left_shoulder.x + byName.right_shoulder.x) / 2;
    ...
    if (track._prev) {
      ...
      verticalVelocity = -((hipY - track._prev.hipY) / dt);
      let sum = 0;
      for (const p of points) {
        const prev = track._prev.byName[p.name];
        if (prev) sum += Math.hypot(p.x - prev.x, p.y - prev.y);
      }
      motion = (sum / points.length) / dt;
    }

    track._smoothMotion = lerp(track._smoothMotion, motion, MOTION_SMOOTHING);
    track._prev = { hipY, byName };

    const f = track.features;
    f.verticalVelocity = verticalVelocity;
    f.motionMagnitude = track._smoothMotion;
    f.bodyAngle = bodyAngle;
    f.groundDurationMs = hipY >= CONFIG.THRESHOLDS.groundLevelY ? f.groundDurationMs + dtMs : 0;
    f.timeSinceMovementMs =
      track._smoothMotion < CONFIG.THRESHOLDS.immobileMotion ? f.timeSinceMovementMs + dtMs : 0;
    return f;
  }

  /**
   * Advance the simulation and return normalised person records.
  ...
  function update(dtMs) {
    ...
    for (const track of tracks.values()) {
      if (track.external) { out.push(externalRecord(track)); continue; }
